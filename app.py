...
with st.sidebar:
    ...
    token = st.text_input("MONDAY_API_TOKEN", value=os.getenv("MONDAY_API_TOKEN", ""), type="password")
    work_board_id = st.text_input("WORK_ORDERS_BOARD_ID", value=os.getenv("WORK_ORDERS_BOARD_ID", ""))
    deals_board_id = st.text_input("DEALS_BOARD_ID", value=os.getenv("DEALS_BOARD_ID", ""))
    model = st.text_input("OPENAI_MODEL", value=os.getenv("OPENAI_MODEL", "gpt-5.6-luna"))
    mode = st.radio("Data source", ["Monday.com", "Demo XLSX"], horizontal=True)
    refresh = st.button("Refresh data")

if "snapshot" not in st.session_state or refresh:
    if mode == "Demo XLSX":
        st.info("Demo mode: upload the two supplied XLSX files. Monday.com mode remains available for the final hosted integration.")
        work_file = st.file_uploader("Work Orders XLSX", type=["xlsx"], key="work_demo")
        deals_file = st.file_uploader("Deals XLSX", type=["xlsx"], key="deals_demo")
        if work_file and deals_file:
            try:
                work_raw = xlsx_to_raw(work_file, "work_orders")
                deals_raw = xlsx_to_raw(deals_file, "deals")
                st.session_state.snapshot = {"work": normalize_board(work_raw, "work_orders"), "deals": normalize_board(deals_raw, "deals"), "work_meta": work_raw["meta"], "deals_meta": deals_raw["meta"]}
        ...
        try:
            ...
            with st.spinner("Reading Monday.com boards..."):
                ...
                st.session_state.snapshot = {
                    "work": work, "deals": deals,
                    "work_meta": work_raw["meta"], "deals_meta": deals_raw["meta"]
                }
            st.success("Connected to Monday.com.")
        except Exception as e:
            st.error(f"Could not load Monday.com: {e}")
    else:
        st.info("Enter the Monday API token and the two board IDs to start, or switch to Demo XLSX mode.")

snapshot = st.session_state.get("snapshot")

if snapshot:
    work, deals = snapshot["work"], snapshot["deals"]

    c1, c2, c3 = st.columns(3)
    c1.metric("Work orders", len(work))
    c2.metric("Deals", len(deals))
    c3.metric("Open deals", int((deals.get("deal_status","").astype(str).str.lower() == "open").sum()) if "deal_status" in deals else 0)
    ...
    selected = st.selectbox("Example", ["Custom question"] + examples)
    question = st.text_area("Founder question", value="" if selected == "Custom question" else selected, height=90)

    if st.button("Analyze", type="primary") and question.strip():
        with st.spinner("Analyzing pipeline and operations..."):
            try:
                context = build_context(work, deals, question)
                result = answer_question(question, context, model=model)
                st.markdown(result)
            except Exception as e:
    ...
    with st.expander("Monday board configuration"):
        st.json({"work_orders": snapshot["work_meta"], "deals": snapshot["deals_meta"]})
else:
    st.markdown("""
